...
const { tutorEntry, idEntry } = require('@validations');
...
module.exports = {
  ...
  async createTutor(req,res) {
    try {
      ...
      const { error } = tutorEntry.createTutorValidation(req.body);
      if(error) return res.status(400).json({
        error: true,
        statusCode: 422,
        message: messages.generalMessages.bad_request,
      ...
      const { error:dataError, statusCode, message, validationErrors, data } = await create(req.body, req.file);
      if(dataError) {
        return res.status(statusCode).json({
          error: dataError,
          statusCode,
  ...
  async update(req,res) {
    try {
      // Joi validation
      const { error } = tutorEntry.updateTutorValidation({id:req.params.id, ...req.body});
      if(error) return res.status(400).json({
        error: true,
        statusCode: 422,
        message: messages.generalMessages.bad_request,
